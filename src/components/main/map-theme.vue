<template>
  <div>
    ...
    <span>
      <i class="el-icon-view" @click="open"></i>
    </span>
  </div>
</template>
...
export default {
  ...
  watch: {
    ...
    text: function (val) {
      ...
      if (val == "GreenField") this.$store.commit("SET_THEME_NAME", mapStyle);
      //改变个性样式
      else if (val == "black") this.$store.commit("SET_THEME_NAME", mapblack);
      else if (val == "Freshblue")
        this.$store.commit("SET_THEME_NAME", mapFreshblue);
      else if (val == "") this.$store.commit("SET_THEME_NAME", val);
    },
  },
  computed: {
    //计算属性 取存在状态库中的值
