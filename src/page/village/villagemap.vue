  ...
  <div class="m_center2">
    <div class="vm_center">
      ...
      <el-button
        type="primary"
        size="mini"
        style="background-color: #1968ab; border: #1968ab solid 1px"
        v-on:click="clear"
        round
        >清空</el-button
      >
    ...
    >
      ...
      <bm-info-window
        ...
        @open="infoWindowOpen"
        >党员值班人:{{ infowindow.uname }} <br />组别:{{ infowindow.grouptype
        }}<br />值班地点:{{ infowindow.address }}<br />值班时间:{{
          infowindow.worktime
        }}<br />值班口号:{{ infowindow.content }} </bm-info-window
      >>
      <bm-boundary
        name="长沙县"
        :strokeWeight="2"
...
export default {
  ...
  methods: {
    ...
    get: function () {
      //3000个终端点
      //发送get请求
      this.$http.get("api/pwork/five").then(
        function (res) {
          for (let i = 0; i < res.data.data.length; i++) {
            for (let j = 0; j < mapvillage.length; j++) {
              if (mapvillage[j].address == res.data.data[i].address) {
                //与地区json匹配赋予经纬度
                const pworkdata = {
                  address: res.data.data[i].address,
                  uname: res.data.data[i].uname,
                  grouptype: res.data.data[i].grouptype,
                  worktime: res.data.data[i].worktime,
                  content: res.data.data[i].content,
                  lng: mapvillage[j].lng,
                  lat: mapvillage[j].lat,
                };
