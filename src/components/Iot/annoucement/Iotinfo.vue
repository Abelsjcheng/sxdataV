  ...
  <div>
    ...
    <b-modal ref="mycontent" hide-footer :title="infocontent.type">
      ...
      <div class="d-block text-center">
        <h4 v-html="infocontent.content"></h4>
        <!--v-html 输出文本对带html标签进行解析 -->
      </div>
      <h6 style="text-align: right">
        {{ infocontent.reldate }} {{ infocontent.adduname }}
      </h6>
      <b-btn class="mt-3" variant="outline-danger" block @click="hidecontent"
        >关闭</b-btn
      >
    </b-modal>
  </div>
...
export default {
  ...
  methods: {
    showcontent(message) {
      ...
      this.infocontent = {
        type: message.type,
        reldate: message.reldate,
        title: message.title,
        content: message.content,
        adduname: message.adduname,
        pic: "",
      };
    ...
    get: function () {
      //发送get请求
      this.$http.get("api/policy/ten").then(
        function (res) {
          for (let i = 0; i < res.data.data.length; i++) {
            this.info = {
              ...
              reldate: res.data.data[i].reldate,
              content: res.data.data[i].content,
              adduname: res.data.data[i].adduname,
